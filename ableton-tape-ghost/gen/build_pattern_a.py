...
p.add("buf_L", text=f"buffer~ tg_a_buf_L 1 {BUF_MS}", col=1, w=200)
p.add("buf_R", text=f"buffer~ tg_a_buf_R 1 {BUF_MS}", col=1, w=200)
p.add("rec_L", text="record~ tg_a_buf_L @loop 1", col=1, w=200)
p.add("rec_R", text="record~ tg_a_buf_R @loop 1", col=1, w=200)
...
live_ui("ui_mix", "live.slider", 140, 40, "TG_A_Mix", "Mix (原音->過去)", "Mix", 0.0, 1.0, 0.0)
live_ui("ui_jump", "live.dial", 90, 90, "TG_A_Jump", "Jump (秒前)", "Jump", 0.0, 120.0, 2.0)
live_ui("ui_rewind", "live.toggle", 50, 30, "TG_A_Rewind", "Rewind", "Rewind")
live_ui("ui_rwspeed", "live.dial", 90, 90, "TG_A_RewindSpeed", "Rewind Speed", "RwSpd", 0.1, 6.0, 1.0)
live_ui("ui_pitch", "live.dial", 90, 90, "TG_A_Pitch", "Pitch (半音, 微分音可)", "Pitch", -24.0, 24.0, 0.0)
live_ui("ui_loop", "live.button", 50, 30, "TG_A_LoopMark", "Loop Mark (2x click)", "LoopMk")
live_ui("ui_loopshift", "live.button", 50, 30, "TG_A_LoopShift", "Loop Shift (further back)", "LoopSh")
live_ui("ui_loopshiftsec", "live.dial", 90, 90, "TG_A_LoopShiftSec", "Loop Shift Amount (sec)", "ShAmt", 0.5, 60.0, 5.0)
live_ui("ui_miracle", "live.toggle", 50, 30, "TG_A_Miracle", "Miracle (cut-up)", "Miracle")
live_ui("ui_mwindow", "live.dial", 90, 90, "TG_A_MiracleWindow", "Miracle History Window (sec)", "MWin", 2.0, 300.0, 45.0)
live_ui("ui_mdensity", "live.dial", 90, 90, "TG_A_MiracleDensity", "Miracle Density", "MDens", 0.0, 1.0, 0.5)
...
p.add("jump_div", text=f"/ {BUF_SEC}.", col=2, w=90)
p.add("jump_pack", text="pack 0. 80", col=2, w=90)
p.connect("ui_jump", 0, "jump_div", 0)
p.connect("jump_div", 0, "jump_pack", 0)
p.connect("jump_pack", 0, "offset_line", 0)
...
# ---------------------------------------------------------------
p.add("cm_loop", cls="comment", comment_text="=== LOOP ENGINE (manual 2-click + Loop Shift + Miracle share this) ===", col=3, w=460)
...
p.add("val_loopstart_w", text="value tg_a_loopstart", col=3, w=140)
...
p.add("val_loopend_w", text="value tg_a_loopend", col=3, w=140)
...
p.add("val_loopstart_r1", text="value tg_a_loopstart", col=3, w=140)
...
# ---------------------------------------------------------------
# Loop shift button: move both loop points back by ShiftSec, re-engage
# ---------------------------------------------------------------
p.add("val_loopstart_r2", text="value tg_a_loopstart", col=4, w=140)
p.add("val_loopend_r2", text="value tg_a_loopend", col=4, w=140)
p.add("shift_ready_t", text="t b b", col=4, w=70)
p.connect("ui_loopshift", 0, "shift_ready_t", 0)
p.connect("shift_ready_t", 1, "val_loopend_r2", 0)
p.connect("shift_ready_t", 0, "val_loopstart_r2", 0)

p.add("shift_div", text=f"/ {BUF_SEC}.", col=4, w=90)
p.connect("ui_loopshiftsec", 0, "shift_div", 0)

p.add("shift_end_expr", text="expr $f1-$f2-floor($f1-$f2)", col=4, w=220)
p.connect("val_loopend_r2", 0, "shift_end_expr", 0)
p.connect("shift_div", 0, "shift_end_expr", 1)

p.add("shift_start_expr", text="expr $f1-$f2-floor($f1-$f2)", col=4, w=220)
p.connect("val_loopstart_r2", 0, "shift_start_expr", 0)
p.connect("shift_div", 0, "shift_start_expr", 1)

p.add("val_loopend_w2", text="value tg_a_loopend", col=4, w=140)
p.add("val_loopstart_w2", text="value tg_a_loopstart", col=4, w=140)

# order: shiftedEnd must land in loop_len_expr's cold inlet1 BEFORE shiftedStart hits hot inlet0.
# val_loopend_r2 was banged first (shift_ready_t outlet1 fires first) so shift_end_expr's result
# is ready before shift_start_expr's result (banged second), giving the correct arrival order.
p.connect("shift_end_expr", 0, "val_loopend_w2", 0)
p.connect("shift_end_expr", 0, "loop_len_expr", 1)
p.connect("shift_start_expr", 0, "val_loopstart_w2", 0)
p.connect("shift_start_expr", 0, "loop_len_expr", 0)
p.connect("shift_start_expr", 0, "loopphase_add", 1)
...
p.add("val_loopstart_wM", text="value tg_a_loopstart", col=6, w=140)
...
p.add("val_loopend_wM", text="value tg_a_loopend", col=6, w=140)
...
p.add("val_loopstart_rM", text="value tg_a_loopstart", col=6, w=140)
...
p.add("wave_L", text="wave~ tg_a_buf_L", col=7, w=140)
p.add("wave_R", text="wave~ tg_a_buf_R", col=7, w=140)
...
data = p.build()
with open("pattern_a.amxd", "w", encoding="utf-8") as f:
